   ...
   const theme = {
    ...
    return (
        <div className='h-full'>
            {<Card
                className={`text-white  md:w-300  border-0 rounded-md bg-[#0000005c] bg-no-repeat bg-blend-multiply  flex-wrap flex flex-col`}
                style={{
            ...
            >
                { props.title ? (<div>
                    ...
                    <h1 className='text-2xl md:text-xl items-center gap-2 flex font-light '>{props.title} <span className='text-black rounded-sm px-4 py-[1px] text-xs bg-white'>{props.tag}</span> </h1>
                    <h1 className='text-sm md:text-xs  font-extralight'>{props.description}</h1>
                </div>

                <div className="flex gap-3  my-5 flex-row flex-wrap">
                    <p className='px-3 py-2 md:px-2 md:text-sm md:py-1 bg-white w-fit h-fit text-black'>{props.RuleBook}</p>
                ...
                <div className="flex flex-row md:flex-col w-full h-full ">
                    ...
                    <div className="flex md:w-full justify-center  items-center flex-col w-1/2 p-10 h-full">
                        <form className='w-full gap-3 p-5 rounded-xl shadow-2xl shadow-slate-800 bg-transparent backdrop-blur-sm h-full flex-col flex justify-center items-center' >
                            ...
                            <Input value={Data.fullName} name='fullName' onChange={changeHandler}  required type="text" className='outline-none w-2/3   rounded-lg border-2 border-gray-400 px-3 py-1 placeholder:text-gray-400 text-black'  placeholder='Full Name' />
